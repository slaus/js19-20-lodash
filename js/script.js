$(function() {
  ...
  $('.jcarousel')
    .on('jcarousel:create jcarousel:reload', function() { //Responsive дизайн
      var widthImage = $(this).innerWidth(),
      heightImage = widthImage * 0.4,
      widthSlider = widthImage * $('.jcarousel__item').length;

      $(this).jcarousel('items').css({'width': widthImage + 'px', 'height': heightImage + 'px'});
      $('.jcarousel__slider').css('width', widthSlider + 'px')
    })
  ...
  .jcarouselPagination({
    item: function(page) {
      $('a.jcarousel_page:first').addClass('active');
  ...
  $('#accordion h3').on('click', function() {
    $('h3.panel__title').removeClass('panel__title--active');
    $('div.panel__text').removeClass('panel__text--active');
    $(this).addClass('panel__title--active');
    $(this).next('.panel__text').addClass('panel__text--active');
    return false;
  });
...
console.log('Массив всех людей, не сортированный                     ', names);

skillsArray = _.sortBy(_.union(_.flatten(_.map(json, 'skills'))));
console.log('Массив скиллов всех людей, отсортированных по алфавиту, без повторов ', skillsArray);

namesArray = _.forEach(_.map(_.sortBy(json, 'friends'), 'name'), result);
console.log('Массив имен людей, отсортированных в зависимости от количества их друзей ', namesArray);

friendsArray = _.sortBy(_.map(_.sortBy(json, 'friends.length'), 'name'));
console.log('Массив всех друзей всех пользователей, отсортированный по алфавиту', friendsArray);
